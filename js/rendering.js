document.addEventListener('DOMContentLoaded', async () => {
	try {
		const res = await fetch('../usuario-data.php', { credentials: 'same-origin' });
		...
		const fullName = [user.nombre, user.apellido].filter(Boolean).join(' ') || user.nombre || user.nombre_completo || '';
		...
		if (fullName) {
			...
			if (profileImg) {
				profileImg.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(fullName)}&background=FF0000&color=fff&size=80`;
			}
		}
		...
		setInfoItemByLabel('DNI', user.dni || user.numero_documento || user.documento);
		setInfoItemByLabel('Teléfono', user.telefono || user.celular || user.telefono_movil);
		setInfoItemByLabel('Email', user.correo || user.email);
		setInfoItemByLabel('Dirección', user.direccion || user.domicilio || user.direccion_fiscal);

		// Actualizar tarjetas/productos por etiquetas internas
		const productCards = document.querySelectorAll('.productos-grid .product-card');
		productCards.forEach((card) => {
			...
			// Saldo disponible
			if (labelText.includes('saldo') && (user.saldo || user.balance || user.saldo_disponible)) {
				amountEl.textContent = `S/ ${user.saldo || user.balance || user.saldo_disponible}`;
			}

			// Monto del crédito
			if (labelText.includes('monto') && (user.monto_credito || user.credito_monto)) {
				amountEl.textContent = `S/ ${user.monto_credito || user.credito_monto}`;
			}

			// Línea disponible (tarjeta)
			if (labelText.includes('línea') && (user.linea_disponible || user.linea)) {
